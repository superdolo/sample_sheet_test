...
class Sample:
	...
	def parse_sample(self,file):
		...
		clean_df = dirty_df.apply(lambda x: x.astype(str).str.strip().str.lower())
		return clean_df
	def __init__(self,file_name):
		xlsx = self.parse_sample(file_name) #Here we parse the entire xlsx
		print(xlsx)
		def generate_dataset(section_name): #write a function to find the starting positions of each section of data section_names = ['[Header]','[Reads]','[Settings]','[Data]']
	...
	def validate_dates(self):
		if datetime.datetime.strptime(self.experiment_name[:8],'%Y%m%d') == datetime.datetime.strptime(self.experiment_date,'%Y-%m-%d %H:%M:%S'):
			...
			print('! Experiment Date: '+str(self.experiment_date)+' DOES NOT MATCH Experiment Name: '+ self.experiment_name)
			#raise ValueError('! Experiment Date: '+str(self.experiment_date)+' DOES NOT MATCH Experiment Name: '+ self.experiment_name)('Experiment Date: '+str(self.experiment_date)+' matches Experiment Name: '+ self.experiment_name)('! Experiment date does not match experiment name')
